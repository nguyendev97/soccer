...
import { Button, CheckmarkIcon, Input, LinkExternal, Text, Toggle, useTooltip } from '@pancakeswap/uikit'
...
const RowWrapper = styled(Row)<{ active: boolean; hasActiveTokens: boolean }>`
  background-color: ${({ active, theme }) => (active ? `${theme.colors.input}` : 'transparent')};
  border: solid 1px;
  border-color: ${({ active, theme }) => (active ? theme.colors.input : theme.colors.tertiary)};
  transition: 200ms;
  align-items: center;
  padding: 1rem;
  border-radius: 4px;
...
const ListRow = memo(function ListRow({ listUrl }: { listUrl: string }) {
  ...
  return (
    ...
    >
      ...
      <Column style={{ flex: '1' }}>
        <Row>
          <TextSymbol>{list.name}</TextSymbol>
        </Row>
        <RowFixed mt="4px">
          <Text fontSize="15px" color="textSubtle" mr="6px" textTransform="lowercase">
            ...
            <RowBetween>
              <RowFixed>
                ...
                <AutoColumn gap="4px" style={{ marginLeft: '20px' }}>
                  <TextSymbol>{tempList.name}</TextSymbol>
                  <Text fontSize="15px" color="textSubtle" small textTransform="lowercase">
